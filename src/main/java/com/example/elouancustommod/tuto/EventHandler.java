...
import static com.example.elouancustommod.registries.ModRecipes.RIGHT_CLICK_BLOCK_RECIPE_TYPE;
...
public class EventHandler {
  ...
  public static void useItemOnBlock(UseItemOnBlockEvent event) {
    // Skip if we are not in the block-dictated phase of the event. See the event's javadocs for
    // details.
    ...
    Optional<RecipeHolder<RightClickBlockRecipe>> optional =
        recipes.getRecipeFor(
            // The recipe type.
            RIGHT_CLICK_BLOCK_RECIPE_TYPE.get(), input, level);
    ItemStack result =
        optional
